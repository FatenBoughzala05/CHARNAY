    ...
    <Card>
      ...
      <Card.Body>
        <Link to={`/product/${product.slug}`} style={{ textDecoration: 'none' }}>
          <Card.Title className='itemcolor' >{product.name}</Card.Title>
        </Link>
        <Rating rating={product.rating} numReviews={product.numReviews} />
        <Card.Text>{product.price.toFixed(3)} TND</Card.Text>
        {product.countInStock === 0 ? (
          ...
          <AddToCartButton addToCartHandler={() => addToCartHandler(product)} />
